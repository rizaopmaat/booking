...
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('admin.booking-options.create') }}" class="bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded">
            {{ __('Add New Option') }}
        </a>
    </div>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse ($options as $option)
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {{ $option->price_type === 'per_person' ? __('Per Person') : __('Fixed') }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <a href="{{ route('admin.booking-options.edit', $option) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">{{ __('Edit') }}</a>
                            <form action="{{ route('admin.booking-options.destroy', $option) }}" method="POST" class="inline-block" onsubmit="return confirm('{{ __("Are you sure you want to delete this option?") }}');">
                                @csrf
                                @method('DELETE')
